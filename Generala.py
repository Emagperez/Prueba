...
def calcular_repetidos(dados):
    ...
    for dado in dados:
        ...
        repetidos[index] += 1
    return repetidos
def buscar_repetido(juego, repetidos, cantidad_repetidos, estricto=False):
    encontre = False
    for repetido in repetidos:
        if juego == "escalera" or juego == "poker" or juego == "generala":
...
def calcular_puntos(numero_lanzamiento, dados, juego):
    ...
    if juego == "generala":
        ...
        if buscar_repetido(juego, repetidos, 5):
            puntos = 50
    elif juego == "poker":
        repetidos = calcular_repetidos(dados)
        if buscar_repetido(juego, repetidos, 4):
            puntos = 40
            if numero_lanzamiento == 1:
                puntos += 5
    elif juego == "full":
        repetidos = calcular_repetidos(dados)
        if buscar_repetido(juego, repetidos, 3) and buscar_repetido(juego, repetidos, 2):
            puntos = 30
            if numero_lanzamiento == 1:
                puntos += 5
        else:
...
def main():
    ...
    while juego.esta_jugado:
        ...
        print(juego.tabla_puntos.Puntaje)
